...
#reader(lib "htdp-beginner-abbr-reader.ss" "lang")((modname |firefighting copy|) (read-case-sensitive #t) (teachpacks ((lib "batch-io.rkt" "teachpack" "2htdp") (lib "universe.rkt" "teachpack" "2htdp") (lib "image.rkt" "teachpack" "2htdp") (lib "itunes.rkt" "teachpack" "2htdp"))) (htdp-settings #(#t constructor repeating-decimal #f #t none #f ((lib "batch-io.rkt" "teachpack" "2htdp") (lib "universe.rkt" "teachpack" "2htdp") (lib "image.rkt" "teachpack" "2htdp") (lib "itunes.rkt" "teachpack" "2htdp")) #f)))
; FIRE-FIGHTING
...
(define (place-waters alop scene)
  (cond
    ...
    [else (place-image
           WATER
           (physical-location (posn-x (first alop)))
           (physical-location (posn-y (first alop)))
           (place-waters (rest alop) scene))]))
...
(define (remove-fire f alop)
  (cond
    ...
    [else
     (if
      (equal? f (first alop))
      (remove-fire f (rest alop))
      (cons (first alop) (remove-fire f (rest alop))))]))
...
(define (end? f)
  (not (plane-valid?
        (move-plane (ff-plane f) "up"))))

; PLANE-VALID?
; Posn -> Boolean
; Check if the given plane posn is valid

(check-expect
 (plane-valid? (make-posn 3 5))
 #true)

(check-expect
 (plane-valid? (make-posn 0 -2))
 #false)

(define (plane-valid? p)
  (>= (posn-y p) -1))
